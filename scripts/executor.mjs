...
try{
 do{
  let status;
  try{status=await executeCycle(provider,signer,c);}catch(error){
  ...
  atomicWrite('output/executor/status.json',{...status,checkedAt:new Date().toISOString()});console.log(JSON.stringify(status));
